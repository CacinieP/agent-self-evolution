...
python reflexion.py --task "写函数 fibonacci(n): 返回第 n 个斐波那契数" --trials 3
```

## 🧠 实现要点

- **真实环境反馈**:`run_tests()` 在隔离命名空间 `exec` 代码并跑测试用例,失败诊断作为反馈信号。
...
3. **对比评测**:对比 `--trials 0`(无记忆)与 `--trials 3`(有记忆)的通过率。
4. **与 Self-Refine 串联**:先自评微调,再环境反馈,体会两种反馈的差异。

## ⚠️ 安全提示

`run_tests()` 使用 `exec/eval` 执行模型生成的代码——**仅用于本地原型自测**。
切勿直接运行不信任模型产出的代码;真实使用请加沙箱(如容器/受限解释器)。
